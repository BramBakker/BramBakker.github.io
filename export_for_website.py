"""
Turns your corpus CSVs into the four JSON files the website reads from /data.
...
# (filename substring, display label) for each questioned document
QUESTIONED_MATCHES = [
...
def build_ngrams_json() -> dict:
    ...
    X, doc_names, feature_names = ngram_mod.build_ngram_matrix(doc_text)
...
def build_calibration_json() -> dict:
    lines_by_doc, _ = rhyme_mod.load_corpus(INPUT_FOLDER, TEXT_COLUMN, rhyme_mod.AUTHOR_COLUMN)
    ...
    doc_text = ngram_mod.load_corpus(INPUT_FOLDER, TEXT_COLUMN)
    X, doc_names_n, _ = ngram_mod.build_ngram_matrix(doc_text)
...
def run() -> None:
    ...
    print("Building null_calibration.json ...")
    calibration = build_calibration_json()
    (OUTPUT_FOLDER / "null_calibration.json").write_text(json.dumps(calibration, ensure_ascii=False, indent=1))

    print(f"Done. Wrote rhymes.json, ngrams.json, null_calibration.json to {OUTPUT_FOLDER}/")
