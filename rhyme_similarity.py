...
QUESTIONED_MATCH = "voortzetting"    # <-- the candidate/disputed manuscript
...
def run() -> dict:
    ...
    if SAVE_PLOT:
        # Note: no automatic "best match" marker anymore -- only KNOWN_MATCHES
        # reference-document comparisons get drawn.
        markers = {}
        for known_idx in known_idxs:
            if known_idx != questioned_idx:
                markers["Similarity Cornicke and Voortzetting"] = sim_matrix[questioned_idx, known_idx]
