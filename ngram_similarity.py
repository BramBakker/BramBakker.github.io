...
HIGHLIGHT_PAIRS = [
    ("cronycke_van_brabant", "voortzetting")
]
...
EXCLUDE_WORDS = [
    "brabant","Brabant", "hertoghe", "greve"
]
# --------------------------------------------------------------------------
# Loading
# --------------------------------------------------------------------------
...
def build_ngram_matrix(doc_text: dict[str, str], exclude_words: list[str] = None):
    doc_names = list(doc_text.keys())
    documents = [doc_text[d] for d in doc_names]

    # Convert the exclude list to a stop_words parameter
    stop_words_list = exclude_words if exclude_words else None

    vectorizer = TfidfVectorizer(
        lowercase=True, 
        ngram_range=NGRAM_RANGE,
        token_pattern=r"(?u)\b\w+\b",
        stop_words=stop_words_list,  # <--- Excludes any n-gram containing these words
...
def run() -> dict:
    ...
    # Pass the exclusion list here:
    X, doc_names, feature_names = build_ngram_matrix(doc_text, EXCLUDE_WORDS)
    ...
    if SAVE_PLOT:
        # Note: no automatic "best match" marker anymore -- only the pairs you
        # explicitly listed in HIGHLIGHT_PAIRS get drawn.
        markers = {}
        ...
        for a_match, b_match in HIGHLIGHT_PAIRS:
            ...
            label = "Similarity, Cornicke and Voortzetting"
